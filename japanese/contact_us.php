	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			...
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">電子メール * : </label>
	                    <?php if($log){ ?>
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">電話 : </label>
	                    <input class="form-control" name="phone" />
                	</div>  
				</div> 
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">件名 * : </label>
	                    <input required class="form-control" name="subject" />
                	</div>
				</div> 
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">メッセージ * : </label>
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("メッセージ","1日に2つ以上のメッセージを追加することはできません", "warning");</script>'; 
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    			echo '<script>swal("メッセージ","メッセージを送信しました", "success");</script>';
    		}else{
    			echo '<script>swal("メッセージ","メッセージは正常に送信されませんでした", "warning");</script>'; 
    		}
    	}   
   		 }else{//recap not ok
   		 	echo '<script>swal("ロボット","私はロボットではない", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("ロボット","私はロボットではない", "warning");</script>';
	    } 
