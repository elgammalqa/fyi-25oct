    ...
    <div class="content container">
        ...
        <div class="row source_top_row">
            ...
            <div class="col-xs-6 col-sm-4 col-md-2">
                <a href="favorite_countries.php" class="btn btn-md btn-primary <?=(!stristr($page,'user_')) ? 'selected' : '';?>">全ての国々</a>
            </div>
            <div class="col-xs-6 col-sm-4 col-md-2">
                <a href="user_favorite_countries.php" class="btn btn-md btn-primary <?=(stristr($page,'user_')) ? 'selected' : '';?>">私の情報源</a>
            </div>
        </div>

        <div class="container stats"><br><h1   >国のリスト : </h1><hr></div>
        <div class="container-fluid">
         ...
         foreach($req as $country){ 
          ...
          ?> 
            <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
             ...
             <?php } 
              ...
              if(isset($_COOKIE['japanese_sources'])){ //there is a cookie
          ...
          <?php }  //for  ?> 
             ...
             <input type="submit" class="green" name="Enregistrer" value="保存する" style="font-size: 26px; margin-right: 60px; " > 
            </form> 
          </div>   
        </div>   
            <?php  
         ...
         $GLOBALS['nb_sources_not_checked']+=v4Utilisteurs::nb_rss_sources_of_country($country['country']);
              ...
              if($GLOBALS['nb_sources_not_checked']>=5){//ok we can continue
                  ...
                  if($log==false){
                      if(isset($_COOKIE['japanese_sources'])){
                      setcookie('japanese_sources', json_encode($data),time()+31556926 ,'/');
                    }else{
                      setcookie('japanese_sources', json_encode($notchecked),time()+31556926 ,'/'); 
                    }
                  }
                  ?>
                ...
                echo "<script> alert('少なくとも5つの情報源に従う必要があります'); </script>";
              }  
