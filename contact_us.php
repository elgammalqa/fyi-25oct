...
<script async defer data-website-id="afc1b19c-5319-4097-8747-3b05933578c7" src="https://www.fyipress.net/umami.js"></script> 
	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			<div class="container"> 
			<h3 class="fcontact" >Contact Us</h3>
...
<?php if (isset($_POST['send'])) {
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("Message","You can not add plus of 2 messages per day", "warning");</script>'; 
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    		echo '<script>swal("Message","message successfully sent","success");</script>';
    		}else{
    		echo '<script>swal("Message","message successfully does not sent","warning");</script>'; 
    		}
    	} 
   		 }else{//recap not ok
   		 	echo '<script>swal("Robot","Check I am not robot", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("Robot","Check I am not robot", "warning");</script>';
	    } 
