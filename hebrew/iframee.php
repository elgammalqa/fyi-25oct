		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-4 sidebar" id="sidebar" style="margin-top: 60px;"> 
					...
					if ($nb_comments!=0) {  ?>
						<aside>  <br> 
							<h1 class="aside-title xhotnewsTitle"  > חדשות אחרונות   </h1> 
					...
					<!--
					<a onclick="f11()" style="cursor: pointer; font-size: 23px; text-align: right; padding-right: 20px; ">
						כתוב הערה 
					</a> 
				-->
				<?php $countofrep=commentsModel::rss_countTotalOfComments($id_art_actuel);
				echo ' מספר הערות  :  '. $countofrep; ?>
			</h3>
			<hr>
		<?php } ?>
	</form> 
				...
				<div class="comment-list">   
									...
									foreach ($qr as $comment) {   ?>  
										<div style=" margin-bottom: 40px; text-align: right;" >
											...
											commentsModel::rss_comment_already_reported($comment['id'],$user_email) ){ ?>
												<button disabled=""  type="button" class="btn btn-danger xreport2" data-dismiss="modal">   להגיש תלונה    
											</button>
										<?php } ?> 
							...
							<?php if ($log){ ?> 
								<form style="padding-bottom: 50px;" id="fff" enctype="multipart/form-data" method="post" class="row"> 
									<div class="form-group col-md-12" style="text-align: right;">
										<label for="message" style="font-size: 26px;" > : תגובה   </label>
										<textarea style="text-align: right; font-size: 26px;"  maxlength="500" required="required" id="myTextField"  class="form-control" name="message"   placeholder=" שאר תגובה   "></textarea>
									</div>  
									<div class="image-upload form-group col-md-8 col-xs-12 " style="float: right;" >  
									 <label for="file-input" style="font-size: 20px;" >
			                           <img style="cursor: pointer;" width="15%" height="15%" src="images/cam.png"/><span class="newFont" style="float: right; padding-left: 20px;" >   תמונה או וידאו  
			                           </span></label> 
                            ...
                            if($state_logo==1){ 
                            	if($uploadOk==1){  
                            		if (isset($_GET['r'])&&!empty($_GET['r'])) {
                            			...
                            			<?php   }else{  ?>
                            				<script>
                            					swal(" תשומת לב  "," ההערה אינה מתווספת   ","warning")
                            				</script> '; 
                            			<?php   }
                            			...
                            			*/
                            			...
                            			if($checkexistedeja==1) 
                            				$msg.=' תמונה או סרטון כבר קיימים, שנה את התמונה או שם הסרטון ונסה שוב   \n';  
                            			if($size_error==1)
                            				$msg.=' גודל הקובץ צריך להיות קטן מ  '.$maxsize .' MB\n'; 
                            			if($typeimage==1) $msg.='  מצטערים, התמונות והסרטונים מתקבלים רק בפורמטים הבאים: jpg, jpeg, png, gif, ac-3, mp3, mp4, flk, mkv, mpeg & webm  \n'; 
                            			if($fin==1) $msg.=' מצטערים, שגיאה בטעינת הקובץ. \n'; 
                            			$msg.=" ההערה אינה מתווספת   ";  
                            			?>
                            			<script>
                            				swal(" תשומת לב  ","<?php echo $msg;?>","warning")
                            			</script> 
                            			<?php 
                            		}
                            	}else{  
