...
<body class="skin-orange">
	<?php 
	require_once ("fyi_header.php"); 
	if(isset($_GET['id'])&&!empty($_GET['id'])){
		$numpage=$_GET['id']; 
		$nbrfeeds= v5newspublished::nbrfeeds2();
							...
							<div class="row">
								<?php  
									...
									if ($nbrfeeds-$start>=10) {//25
										$nbhnews=4;
										$req=v5newspublished::getspecialfeeds($start,10);
										foreach($req as $news){  
												$linkv="detail.php?id=".$news['id'];
												$typesectionv=$news['type'];    
											?>
											<article class="col-md-11 box article-fw">
												<div class="details">
													<h6><a  style="color: #000;" href="<?php echo $linkv; ?>">
									...
									$req=v5newspublished::getspecialfeeds($start,$nbrfeeds-$start);
									...
									foreach($req as $news){  
											...
											$typesectionv=$news['type'];   
											   ?> 
										<article class="col-md-11 article-fw" style="padding-bottom: 30px;">
											<div class="details">
												<h6><a  style="color: #000;" href="<?php echo $linkv; ?>">
													<?php echo stripslashes($news['title']); ?> 
												</a>
		...
		<div class="col-md-4 sidebar" id="sidebar"> 
			<?php  if(v5newspublished::nbrhotnews()>0){  ?>
				<aside>
					<br>
					<h1  id="ptop" class="aside-title">Recent News</h1>
       ...
       <?php }else{?>
       <script>window.location.replace('fyi.php');</script> 
       <?php } 
   		}else{?>
       <script>window.location.replace('fyi.php');</script> 	
       <?php }  
