  ...
  if(userModel::islogged("Admin")==true){ 
    if(userModel::isLoged_but_un_lock()){
       ...
       if(isset($_GET["art"])&&!empty($_GET["art"])){
         ...
         if (count($_COOKIE)>0)  
          setcookie('fyiplien','a_edit_article.php?art='.$_GET["art"],time()+2592000,"/");
            else   $_SESSION['auth']['lien']="a_edit_article.php?art=".$_GET["art"] ; 
                            ...
                            <label class="control-label col-md-3  col-xs-12" for="first-name"> 
                              ...
                              <div class="form-group">
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                  <button href="a_editnews.php" class="btn btn-primary" type="button">Cancel</button> 
      ...
      if ( $media_update_msg_error==0&&$photo_msg_error==0&&
         $thumbnail_update_msg_error==0 &&$thumbnail_msg_error==0) {
          if ($n->update_news($art)){    
             echo '<script type="text/javascript">  window.location.replace("a_editnews.php"); </script>'; 
