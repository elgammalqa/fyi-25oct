    ...
    if(userModel::isLoged_but_un_lock()){
      ...
      if (count($_COOKIE)>0)  setcookie('fyiplien','all_by_date_and_country.php',time()+2592000,"/");
       ...
       if (isset($_COOKIE['fyipEmail'])&&!empty($_COOKIE['fyipEmail'])){
        ...
        function traffic_By_cn($d11,$d22,$countryyy,$langg){  
            include($langg);
            $requete = $con->prepare("SELECT sum(nb) FROM `visitors` WHERE  `country` ='".$countryyy."' and `date` BETWEEN '".$d11."' and '".$d22."'");
                       $requete->execute();
                        $tbtwodates = $requete->fetchColumn();
                        return  $tbtwodates ;  
              }

               function table1($date1,$date2,$countryyy,$langg,$l){
                  include($langg);  
                  if(traffic_By_cn($date1,$date2,$countryyy,$langg)!=null){  ?>
                    <tr class="headings"> 
                          <th class="column-title" > <?php echo $l; ?> </th> 
                           <th  class="column-title"> 
                            <span style="color: red; font-size: 15px; "  >
                              <?php echo traffic_By_cn($date1,$date2,$countryyy,$langg); ?>
                            </span>   
                          </th>   
                        </tr> 
          <?php  }}  ?> 
        ...
        <div class="col-md-3 left_col" style="position: fixed;">
          <div class="left_col scroll-view">
            <div class="navbar nav_title" style="border: 0; margin-top: 10px;margin-bottom: 10px;">
              <a href="all_cpanel.php" class="site_title"><img style="width: 40px; height: 40px; margin-left: 5px; " class="img-circle" src="images/fyi.jpeg" ><span> FYI press </span></a>
            </div> 
              ...
              <form method="post" > 
                    <div class="form-group" style="margin-top: 20px; " > 
                     <label  id="mobile" class="control-label col-md-1 col-sm-12 col-xs-12   ">
                      ...
                      <label  id="mobile"  class="control-label col-md-1 col-sm-12 col-xs-12   "> Form : </label> 
                      <div class="col-md-2   col-sm-12 col-xs-12 ">
                       ...
                       <label  id="mobile"  class="control-label col-md-1 col-sm-12 col-xs-12   ">
                        To : </label> 
                      <div class="col-md-2   col-sm-12 col-xs-12 ">
                        <input required="" autocomplete="off"  id="datepicker2" name="date2" 
                           class="form-control "  >
                      </div>   
                         
                     </div>  
                     <button id="mobile" name="search" class="btn btn-success col-md-1 col-sm-12 col-xs-12 "> Search </button>
               </form>
               </div> 
            </div>   
                 <?php 
            if(isset($_POST['search'])){
                if (isset($_POST['date1'])&&isset($_POST['date2'])&&isset($_POST['countries'])) { 
                      if ($_POST['countries']!="Select") {
                        if(strtotime($_POST['date2'])>=strtotime($_POST['date1'])){
                          $countryyy=$_POST['countries']; 
                             $p = explode('-', $_POST['date1']);  $d1=$p[2]."/".$p[1]."/".$p[0]; 
                             $part = explode('-', $_POST['date2']); 
                              $d2=$part[2]."/".$part[1]."/".$part[0];
$date1=$_POST['date1'];$date2=$_POST['date2'];
     $sql_request="SELECT sum(nb) FROM `visitors` WHERE  `country` ='".$countryyy."' and `date` BETWEEN '".$date1."' and '".$date2."'";
     $nb_visitorsTotal=userModel::totl_rss_all($sql_request);
   if ($nb_visitorsTotal!=null) { ?> 
            <div style="margin-top: 2%" class="col-md-6 col-sm-12 col-xs-12 col-md-offset-2">
                 <table class="table table-striped jambo_table bulk_action">
                    ...
                    table1($date1,$date2,$countryyy,$langg,$l);   
              ...
              <table class="table table-striped jambo_table bulk_action">
                      <thead>
                          <tr class="headings"> 
                            ...
                            <th class="column-title"><?php echo $nb_visitorsTotal; ?></th>    
                          </tr>
                      </thead>   
              </table>  
            </div> 
             <?php } else {
   echo '<script>swal("No Visitor","There is No Visitors From '.$countryyy.' Between '.$d1.' and '.$d2.' in all languages","warning")</script>';
                  } 
               } else {
                 echo '<script>swal("Date error!","date2 should be after date1","warning")</script>';  }
              }else{
