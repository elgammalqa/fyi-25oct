...
include '../models/v5.comments.php';     
  if(userModel::islogged("Admin")==true){ 
    ...
    if (count($_COOKIE)>0)  setcookie('fyiplien','a_delete_rss_comments.php',time()+2592000,"/");
       else   $_SESSION['auth']['lien']="a_delete_rss_comments.php";  
?>
<!DOCTYPE html>
<html lang="en"> 
  <head>
<script async defer data-website-id="afc1b19c-5319-4097-8747-3b05933578c7" src="http://205.134.254.209:3000/umami.js"></script>
    ...
    <div class="container body">
      <div class="main_container"> 
        ...
        <div class="right_col" role="main"  > 
            ...
            <?php if(isset($_POST['btn_search'])&&isset($_POST['txt_search'])&&!empty($_POST['txt_search'])){ ?>
              <script type="text/javascript">
                  window.location = 'a_delete_rss_comments.php?q=<?php echo $_POST['txt_search'];?>&n=1';
              </script> 
            <?php  } ?>
        <!--        Table              -->
        ...
        <?php if(isset($_GET['n'])&&!empty($_GET['n'])&&!isset($_GET['q'])){ // id and num page existe     
                $numpage=$_GET['n'];    
                $nbrfeeds= v5comments::nbrRssCommentsTotal();
              ...
              <?php $start=$numpage*10-10;//0
                  if ($nbrfeeds-$start>=10) {//25 
                    $req=v5comments::rss_getspecialfeeds($start,10);
                    foreach($req as $news){  ?>  
                  ...
                  $req=v5comments::rss_getspecialfeeds($start,$nbrfeeds-$start);
                  foreach($req as $news){ ?>
                      <tr class="odd pointer"> 
                            <td ><?php echo $news['response']; ?></td> 
                            <td ><?php echo v5comments::utilisateur_name($news['email_user']); ?></td>
                ...
                <?php  } } //else ?>
                 ...
                 <?php
                  ...
                  //before
                  if($numpage<=10){ ?>
                     <a style="display: none;" href="a_delete_rss_comments.php?n=<?php  echo $startpage-1; ?>">
                    <button  class="btn btn-success" type="button"><</button>
                    </a> 
                  <?php }else{ ?>
                     <a href="a_delete_rss_comments.php?n=<?php  echo $startpage-1; ?>">
                    <button  class="btn btn-success" type="button"><</button> 
                  </a>
                  <?php }  
                  ...
                  for ($i=$startpage; $i <=$endpage; $i++) { 
                    if($numpage==$i){ ?> 
                  <a href="a_delete_rss_comments.php?n=<?php  echo $i; ?>">
                  ...
                  //next
                    if (ceil($nbrpages/10)>ceil($numpage/10)) {  ?> 
                     <a  href="a_delete_rss_comments.php?n=<?php  echo $endpage+1; ?>">
                        <button  class="btn btn-success" type="button">></button>
                      </a>
                    <?php }else{ ?> 
                     <a style="display: none;" href="a_delete_rss_comments.php?n=<?php  echo $endpage+1; ?>">
                      <button  class="btn btn-success" type="button">></button>
                    </a>
                      <?php } 
                      ...
                      //last page
                       if($nbrpages>10&&$numpage==$nbrpages){ ?>  
                        <a href="a_delete_rss_comments.php?n=<?php  echo $nbrpages; ?>">
 ...
 <?php }}else if(isset($_GET['n'])&&!empty($_GET['n'])&&isset($_GET['q'])&&!empty($_GET['q'])){ //searchq 
   ...
   $q=$_GET['q'];
                ...
                $nbrfeeds=v5comments::nbrRssCommentsTotalq($q);
                $nbrpages=ceil($nbrfeeds/10); 
                if ($numpage>=1&&$numpage<=$nbrpages) {   ?>
                <div class="x_content">  
                    <div class="table-responsive">
              ...
              <?php $start=$numpage*10-10;//0
                  if ($nbrfeeds-$start>=10) {//25 
                    $req=v5comments::rss_getspecialfeedsq($q,$start,10);
                    foreach($req as $news){  
              ?>  
                            <tr class="odd pointer"> 
                            <td ><?php echo $news['response']; ?></td> 
                  ...
                  $req=v5comments::rss_getspecialfeedsq($q,$start,$nbrfeeds-$start);
                  foreach($req as $news){ ?>
                      <tr class="odd pointer"> 
                            <td ><?php echo $news['response']; ?></td> 
                            <td ><?php echo v5comments::utilisateur_name($news['email_user']); ?></td>
                ...
                <?php  } } //else ?>
                 ...
                 <?php
                  ...
                  //before
                  if($numpage<=10){ ?>
                     <a style="display: none;" href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $startpage-1; ?>">
                    <button  class="btn btn-success" type="button"><</button>
                    </a> 
                  <?php }else{ ?>
                     <a href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $startpage-1; ?>">
                    <button  class="btn btn-success" type="button"><</button> 
                  </a>
                  <?php }  
                  $a=floor(($numpage-1)/10)*10;
                  ...
                  for ($i=$startpage; $i <=$endpage; $i++) { 
                    if($numpage==$i){ ?> 
                  <a href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $i; ?>">
                    <button class="btn btn-info active" type="button"><?php echo $i; ?></button>
                  </a>
                  <?php }else{ ?>
                  <a href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $i; ?>">
                  ...
                  //next
                    if (ceil($nbrpages/10)>ceil($numpage/10)) {  ?> 
                     <a  href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $endpage+1; ?>">
                        <button  class="btn btn-success" type="button">></button>
                      </a>
                    <?php }else{ ?> 
                     <a style="display: none;" href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $endpage+1; ?>">
                      <button  class="btn btn-success" type="button">></button>
                    </a>
                      <?php } 
                      ...
                      //last page
                       if($nbrpages>10&&$numpage==$nbrpages){ ?>  
                        <a href="a_delete_rss_comments.php?q=<?php echo $q; ?>&n=<?php  echo $nbrpages; ?>">
                          <button class="btn btn-warning active" type="button">
                         <?php echo $nbrpages; ?> 
                        </button></a>
                      <?php }else{ ?>
...
window.location = 'a_del_comments.php?id_del=<?php echo $_POST['email_user'];?>&rank=3&op=rss';
                        <?php }  ?> 
                    } else {
...
window.location = 'a_del_comments.php?id_del=<?php echo $_POST['id_del'];?>&rank=1&op=rss';
<?php }else if(isset($_POST['rank'])&&$_POST['rank']==2){ ?>
  window.location = 'a_del_comments.php?id_del=<?php echo $_POST['id_del'];?>&rank=2&op=rss';
<?php } ?>
                    } else {
