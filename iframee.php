...
<body onload="f10()" class="skin-orange">
	...
	if(isset($_GET['id'])&&!empty($_GET['id']) && isset($_GET['link'])&&!empty($_GET['link']) ){    
		...
		<div class="container">
			<div class="row"> <div class="col-md-8 " style="margin-top: 40px; "> 
						<div class="comments">  
						<form method="post" >
							<?php if ($log==false) { ?>
						...
						<p class="xtitleen" style="text-align: center;"> Comments </p>
						 ...
						 <span class="xtitle2en" >
						 	The published comments do not represent the views of ChatsRun and its employees, but rather the views of those who wrote them.
 						 </span> <br><br>
					    ...
					    <?php $qrss=repliesModel::rss_repliesStartNbres(0,10,$id_art_actuel,$comment['id']);
								  ...
								  <?php } else if(!commentsModel::rss_HisReply($reply['id'],$user_email) && 
								   commentsModel::rss_reply_already_reported($reply['id'],$user_email)){ ?> 
 									<button disabled="" type="button"
 										 class="btn btn-danger xreporten2" data-dismiss="modal">Report
								     </button> 
 									<?php } ?> 
								    </div>   
								   <?php  } // log true ?>
							...
							<form style="padding-bottom: 50px;"  enctype="multipart/form-data" method="post" class="row"> 
								<div class="form-group col-md-12">
									...
									<textarea style="font-size: 18px;" maxlength="500" required="required" id="myTextField"  class="form-control" name="message"  placeholder="Write your Comment ..."></textarea>
								</div>  
								...
								<div class="form-group col-md-4"> 
									...
									<input value="Send Comment" name="send" type="submit" class="btn btn-primary form-control" style="font-size: 20px; padding-top:5px !important; margin-top: 0px !important">
								</div>  
							</form>
								<?php  
							// comment 
                            ...
                            if($state_logo==1){  
                            	if($uploadOk==1){ 
                            		if (isset($_GET['r'])&&!empty($_GET['r'])) {
                            			...
                            			<?php  }else   
                            			echo '<script> swal("Attention!","Comment does not added !","warning"); </script> '; 
                            		...
                            		*/
                            		...
                            		if (isset($_GET['r'])&&!empty($_GET['r'])) {
                            			...
                            			<?php  } else   
                            			echo '<script>  swal("Attention!","Comment does not added !","warning")
                            			</script> ';  
                            		}
                            		/*
										else{ 
