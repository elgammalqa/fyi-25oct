...
<body onload="f10()" class="skin-orange">
		...
		<div class="container">
			<div class="row"> 
					...
					<div class="col-md-8 " style="margin-top: 40px; "> 
						...
						<form method="post" >
							<?php if ($log==false) { ?>
							<h3 class="xlogin">Lütfen <a href="login.php">&nbsp; oturum aç &nbsp;</a>yorum yapmak</h3>  
							<div class="line thin"></div>
						<?php } ?> 

						<p class="xtitleen" style="text-align: center;"> yorumlar </p>
 						<p class="xtitleen"> ihbar : </p>
						 <span class="xtitle2en" >
						 	Yayınlanan yorumlar, Chats Run ve çalışanlarının görüşlerini değil, onları yazanların görüşlerini temsil eder.
 						 </span> <br><br>

						<?php if ($log){ ?>
							<p style="font-size: 22px; ">
								<?php $countofrep=commentsModel::rss_countTotalOfComments($id_art_actuel);
								if ($countofrep<=1) echo $countofrep.' açıklama ';
								else echo $countofrep.' yorumlar ';  ?>  &nbsp; &nbsp; &nbsp;
								<a onclick="f10()" style="cursor: pointer;"> Bir yorum Yaz </a>
							</p> <hr>
						<?php } ?> 
						</form>
                            ...
                            !commentsModel::rss_comment_already_reported($comment['id'],$user_email) ){ ?> 
								<a href="<?php echo 'rss_report.php?link='.$clink.'&id='.$id_art_actuel.'&id_c='.$comment['id']; ?>">
									<button type="button" class="btn btn-danger xreporten1"
											 data-dismiss="modal">Rapor
									</button>
								</a> 
						  <?php }else if(! commentsModel::rss_HisComment($comment['id'],$user_email) && 
                            commentsModel::rss_comment_already_reported($comment['id'],$user_email) ){ ?>
                            <button disabled="" type="button" class="btn btn-danger xreporten2" > 				Rapor 
							</button> 
                                    <?php } ?>    
								  </div>  
								   <?php  } ?>
								  ...
								  <?php } else if(!commentsModel::rss_HisReply($reply['id'],$user_email) && 
								   commentsModel::rss_reply_already_reported($reply['id'],$user_email)){ ?> 
 									<button disabled="" type="button"
 										 class="btn btn-danger xreporten2" data-dismiss="modal">Rapor
								     </button> 
 									<?php } ?> 
								    </div>   
								   <?php  } // log true ?>
			...
			<?php	if($log){  ?>
							<form style="padding-bottom: 50px;"  enctype="multipart/form-data" method="post" class="row"> 
								<div class="form-group col-md-12">
									<label for="message">Yorum Yap <span class="required"></span></label>
									<textarea style="font-size: 18px;" maxlength="500" required="required" id="myTextField"  class="form-control" name="message"  placeholder="yorumunu yaz ..."></textarea>
								</div>  
								<div class="image-upload form-group col-md-8">  
									 <label for="file-input" style="font-size: 18px;">
			                          Resim veya video ekle &nbsp; : &nbsp; <img style="cursor: pointer;" width="15%" height="15%" src="images/cam.png"/>
			                           </label> 
								...
								<div class="form-group col-md-4"> 
									...
									<input value="yorum gönder" name="send" type="submit" class="btn btn-primary form-control" style="font-size: 20px; padding-top:5px !important; margin-top: 0px !important">
								</div>  
							</form> 
								<?php  
							// comment 
                            ...
                            if($state_logo==1){  
                            	if($uploadOk==1){ 
                            		if (isset($_GET['r'])&&!empty($_GET['r'])) {
                            			...
                            			<?php  }else { ?>
                            				<script> 
                            					swal("Dikkat!","Yorum eklenmedi !","warning"); 
                            				</script>
                            				<?php 	}  
                            		}
                            		/*
                                     ...
                                     */
                                            ...
                                            $msg=null;  
                                             if($checkexistedeja==1) $msg.='Dosya zaten mevcut, lütfen dosya adını değiştirin ve tekrar deneyin\n';  
                            if($size_error==1) $msg.='Dosya boyutu en az olmalıdır '.$maxsize .' MB\n'; 
                                            if($typeimage==1) $msg.='Üzgünüm, Sadece Kabul jpg, jpeg, png, gif, ac-3, mp3, mp4, flk, mkv, mpeg & webm Biçim\n'; 
                                            if($fin==1) $msg.='Üzgünüm, Dosya Yükleme Hatası.\n'; 
                                            $msg.="yorum eklenmedi";  
                                            ?>
                                            <script>
                                                 swal("Dikkat!","<?php echo $msg;?>","warning")
                                            </script> 
                            			<?php 
                            		}
                            	}else{  
 ...
 <div class="col-md-4 sidebar" id="sidebar" style="margin-top: 60px;">
			...
			if ($nb_comments!=0) {  ?>
				<aside>
					<br>
					 <h1 class="aside-title">Son haberler</h1> 
