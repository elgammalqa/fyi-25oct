...
<script async defer data-website-id="afc1b19c-5319-4097-8747-3b05933578c7" src="http://205.134.254.209:3000/umami.js"></script> 
	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			<div class="container"> 
			<h3 class="fcontact" >Bize ulaşın</h3>
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">E-posta * : </label>
	                    <?php if($log){ ?>
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">Telefon numarası : </label>
	                    <input class="form-control" name="phone" />
                	</div>  
				</div>
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">konu * : </label>
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">mesaj * : </label>
...
<?php if (isset($_POST['send'])) {
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("mesaj","Günde 2 mesajdan fazla ekleyemezsiniz", "warning");</script>';
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    		echo '<script>swal("mesaj","Mesaj başarıyla gönderildi", "success");</script>';
    		}else{
    		echo '<script>swal("mesaj","mesaj başarıyla gönderilmedi", "warning");</script>'; 
    		}
    	} 
   		 }else{//recap not ok
   		 	echo '<script>swal("Robot","Bir robot olmadığımı kontrol et", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("Robot","Bir robot olmadığımı kontrol et", "warning");</script>';
	    } 
