                         ...
                         // ads  
									if (adsModel::nbr_ads2()>0) {
										...
										<div class=" time2"> 
											...
											echo ads_link_comments($news['id'],$log,$linkv,'açıklama','yorumlar'); 
											?>
                                            <span style="padding-left: 20px; float: left;margin-right: 20px;">
                            <a style="color:black;" target="_blank" href="news_reply.php?news=<?= $news['id'] ?>&r=<?=$news['rank']?>">
                                Cevap hakkı <img src="images/microphone.png">
                            </a>
                        </span>
										</div>
                       		...
                       		//news 
								...
								if($nbrnewstodisplay>0){ 
									...
									foreach($req as $news){  
										...
										<article>
											<?php 	 
											$type=news_publishedModel::typeOfMedia($news["media"]); 	if ($type=="image") { ?>
											...
											<div class="time2">
												...
												<?php echo $source; ?> </a> &nbsp;&nbsp;&nbsp;&nbsp;
												<?php real_news_time($news['pubDate']); 
												echo link_comments($news['id'],$log,$linkv,'açıklama','yorumlar',$news['rank']);
												?>
                                                <span style="padding-left: 20px; float: left; margin-right: 20px;">
                            <a style="color:black;" target="_blank" href="news_reply.php?news=<?= $news['id'] ?>&r=<?=$news['rank']?>">
                                Cevap hakkı <img src="images/microphone.png">
...
<?php if(news_publishedModel::nbrnewsStartCountVideos2()>0){ ?>
	...
	<section  class="best-of-the-week" style="padding-bottom: 0px;padding-top: 0px;">
		<div class="container">
			<h1><div class="text">videolar</div>
			...
			<?php  if(news_publishedModel::nbrNewsBySourceindex($n)>0){ ?>
				...
				<div class="row">
					...
					foreach($req as $news){ 
				        ...
				        $sentence=implode(' ', array_slice(explode(' ', $real_sentence), 0, 30));
						echo $sentence;   
						if ($nb_words>30) { echo '...'; }  
									?>   
									</p>
			...
			<div class="row" > 
				...
				<div class="col-xs-12 col-sm-12 col-md-4  "   style="padding-bottom: 60px;" >  
					<?php if(news_publishedModel::rsscount2()>0||news_publishedModel::nbr_news_without_images2()>0) { 
							$nbnews=$GLOBALS['nb_news_types']*7; 
						?>
						<aside><br>
							<div class="line" style="margin: 0px 0px;"><div>HABERLER</div></div>
