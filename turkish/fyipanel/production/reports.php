  ...
  if(userModel::islogged("Admin")==true){ 
    if(userModel::isLoged_but_un_lock()){
      header('Location:lock_screen.php');
       ...
       if (count($_COOKIE)>0)  setcookie('fyiplien',"reports.php",time()+2592000,"/");
       else   $_SESSION['auth']['lien']="reports.php"; 
?>
<!DOCTYPE html>
<html lang="en">
  <head>
                    ...
                    <table id="example" class="table table-bordered" cellspacing="0" width="100%">
                      ...
                      $news = new reportModel();
                        ...
                        foreach($query as $news){  ?>     
                            <tr >  
                            <td ><?php echo reportModel::getNewsTitle($news['id_news']); ?></td>

                         <?php if ($news['id_comment']!=null){ ?>
                         <td ><?php echo reportModel::getResponseComment($news['id_comment']); ?></td>
                            <?php }else{ ?>
                        <td ><?php echo reportModel::getResponseReply($news['id_reply']); ?></td>
                          <?php }  ?>
            ...
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        ...
                        window.location = 'delete_report.php?id_cr=<?php echo $_POST['id_cr'];?>';
                    } else {
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        swal({
                            ...
                            timer: 100,
                            showConfirmButton: false });
                        window.location = 'delete_report.php?id_v=<?php echo $_POST['id_v'];?>';
                    } else {
                        swal("Cancellation ", "Cancellation successfully ", "info");
                    }
                });
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        swal({
                            ...
                            timer: 100,
                            showConfirmButton: false });
                        window.location = 'delete_report.php?id_r=<?php echo $_POST['id_r'];?>';
                    } else {
                        swal("Cancellation ", "Cancellation successfully ", "info");
                    }
                });
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        swal({
                            ...
                            timer: 100,
                            showConfirmButton: false });
                        window.location = 'delete_report.php?id_comment=<?php echo $_POST['id_comment'];?>';
                    } else {
                        swal("Cancellation ", "Cancellation successfully ", "info");
                    }
                });
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        swal({
                            ...
                            timer: 100,
                            showConfirmButton: false });
                        window.location = 'delete_report.php?id_reply=<?php echo $_POST['id_reply'];?>';
                    } else {
                        swal("Cancellation ", "Cancellation successfully ", "info");
                    }
                });
