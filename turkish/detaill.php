			...
			<div class="container">
				<div class="row">  
					...
					<?php if(news_publishedModel::nbrnewsStartCountVideos2()>0){ ?>
						<div class="line"><div>Şunlar da hoşunuza gidebilir</div></div>
						<div class="row">  
							 <?php  	// news videos   
							   ...
							   <?php if ($log){?>
								...
								!commentsModel::reply_already_reported($reply['id'],$user_email)){ ?>
								   ...
								   <?php }else if(!commentsModel::HisReply($reply['id'],$user_email) && 
								     commentsModel::reply_already_reported($reply['id'],$user_email)){ ?>
								        <button disabled="" type="button" class="btn btn-danger xreporten2" data-dismiss="modal">Rapor
								        </button>
								         <?php } ?> 
                                	...
                                	if (isset($_GET['r'])&&!empty($_GET['r'])) {
                                     ...
                                     <?php   }else   echo '<script>
                                          ...
                                          if($size_error==1) $msg.='Dosya boyutu en az olmalıdır '.$maxsize.' MB\n';
                                             if($checkexistedeja==1) $msg.='Dosya Zaten Var, Lütfen Dosya Adını Değiştirin ve Tekrar Deneyin\n'; 
                                            if($typeimage==1) $msg.='Üzgünüm, Sadece Kabul jpg, jpeg, png, gif, ac-3, mp3, mp4, flk, mkv, mpeg & webm Biçim\n'; 
                                            if($fin==1) $msg.='Maalesef Dosya Yüklenirken Hata Oluştu.\n'; 
                                            $msg.="yorum eklenmedi";  
                                            ?>
                                            <script>
