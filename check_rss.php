						...
						<div class="box-body">
							...
							<form method="post" >
								<div class="form-group">
									<label>Link :</label>
									<input placeholder="Enter rss link ..." required="required" name="link" class="form-control">
								</div> 
								<br>
								<div class="form-group text-right">
									<button name="check" class="btn btn-primary btn-block">Check</button>
								</div>   
							</form>  
			 ...
			 <?php if(isset($_POST['check'])){  
        		  		try{
 							$content = @file_get_contents($_POST['link']);
							  if ($content === false) { ?>
							  	<script> swal("Link !","invalid rss link!","warning");</script>
						    <?php }else{ 
						    //source ok
						    	 $xml = simplexml_load_string($content); 
						    	 foreach ($xml->channel->item as $item) {   
						    	 	//foreach ($item->children() as $child)  {
								    //echo $item[0]."Child node: " . $child['name'] . "<br>";
								    //}
