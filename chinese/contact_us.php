...
try{
	include 'fyipanel/views/connect.php';
 if($con->exec("INSERT INTO `messages` (`id`, `email`, `name`, `subject`, `message`) VALUES
	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			...
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">学科 * : </label>
	                    <input required class="form-control" name="subject" />
                	</div>
				</div> 
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">信息 * : </label>
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("信息","您每天不能添加2条消息", "warning");</script>';
    	}else{ 
    		if(add_message($email,$name,$subject,$message,$phone)){
    	 	echo '<script>swal("信息","消息已成功发送", "success");</script>';
    		}else{
    		echo '<script>swal("信息","消息成功未发送", "warning");</script>'; 
    		}
    	}  
   		 }else{//recap not ok
   		 	echo '<script>swal("机器人","确认我不是机器人", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("机器人","确认我不是机器人", "warning");</script>';
	    } 
