			...
			<div class="container">
				<div class="row"> 
								  	...
								  	<?php }else{ ?>   
										...
										<?php }   ?>  
												...
												<a  href="detail.php?id=<?php echo $news->getid(); ?>" > 
											        <img width="100%" height="100%"  src="<?php echo 'fyipanel/views/image_news/'.$news->getmedia(); ?>" alt="Sample Article">
											    </a>
											    <?php } ?>    
								</div> 
								<?php } ?>     
				 					<div class="xtypedate" >
										<span class="xdateen" >
										发布  <?php real_news_time($news->getpubDate()); ?>
										</span> 
										<a class="xtypeen" >
					...
					<?php if(news_publishedModel::nbrnewsStartCountVideos2()>0){ ?>
						<div class="line"><div>你可能也喜欢</div></div>
						<div class="row">  
							 <?php  	// news videos   
					...
					<div class="col-md-6 col-sm-6 col-xs-12"  >
						...
						<form method="post" >
							<?php if ($log==false) { ?>
							<h3 class="xlogin"> 请 <a href="login.php">&nbsp;登录&nbsp;</a>评论</h3> 
							<div class="line thin"></div>
						<?php } ?> 

						<p class="xtitleen" style="text-align: center;"> 评论 </p>
 						<p class="xtitleen"> 注意 : </p>
						 <span class="xtitle2en" >
						 	发表的评论并不代表Chats Run及其员工的观点，而是代表撰写评论的人的观点。
 						 </span> <br><br>

						<?php if ($log){ ?>
							<p style="font-size: 22px; ">
								<?php $countofrep=commentsModel::countTotalOfComments($id_art_actuel);
								if ($countofrep<=1) echo $countofrep.' 评论 ';
								else echo $countofrep.' 评论 ';  ?>  &nbsp; &nbsp; &nbsp;
								<a onclick="f10()" style="cursor: pointer;"> 留个评论 </a>
							</p> <hr>
						<?php } ?> 
						</form>   
							...
							!commentsModel::comment_already_reported($comment['id'],$user_email) ){ ?>
								<a href="<?php echo 'report.php?id='.$id_art_actuel.'&id_c='.$comment['id']; ?>">
									<button type="button" class="btn btn-danger xreporten1"
											 data-dismiss="modal">报告
									</button>
								</a>
							<?php } else if(!commentsModel::HisComment($comment['id'],$user_email) &&
						 		commentsModel::comment_already_reported($comment['id'],$user_email) ){ ?>
								<button disabled="" type="button" class="btn btn-danger xreporten2" > 			报告 
								</button>
							<?php } ?> 
							   ...
							   <?php if ($log){?>
								...
								!commentsModel::reply_already_reported($reply['id'],$user_email)){ ?>
								   ...
								   <?php }else if(!commentsModel::HisReply($reply['id'],$user_email) && 
								     commentsModel::reply_already_reported($reply['id'],$user_email)){ ?>
								        <button disabled="" type="button" class="btn btn-danger xreporten2" data-dismiss="modal">报告
								        </button>
								         <?php } ?> 
							...
							<form style="padding-bottom: 50px;"  enctype="multipart/form-data" method="post" class="row"> 
								...
								<div class="image-upload form-group col-md-8">  
									 <label for="file-input" style="font-size: 18px;">
			                          添加图片或视频 &nbsp; : &nbsp; <img style="cursor: pointer;" width="15%" height="15%" src="images/cam.png"/>
			                           </label> 
								...
								<div class="form-group col-md-4"> 
									...
									<input value="发送评论" name="send" type="submit" class="btn btn-primary form-control" style="font-size: 20px; padding-top:5px !important; margin-top: 0px !important">
								</div>  
							</form>
							<?php  
							// comment 
                                ...
                                if($uploadOk==1){ 
                                	if (isset($_GET['r'])&&!empty($_GET['r'])) {
                                     ...
                                     <?php   }else   echo '<script>
                                            ...
                                            $msg=null;  
                                          if($size_error==1) $msg.='文件大小必须小于 '.$maxsize.' MB\n';
                                             if($checkexistedeja==1) $msg.='文件已存在，请更改文件名并重试\n'; 
                                            if($typeimage==1) $msg.='对不起，请接受jpg，jpeg，png，gif，ac-3，mp3，mp4，flk，mkv，mpeg和webm格式\n'; 
                                            if($fin==1) $msg.='对不起，上传文件时出错.\n'; 
