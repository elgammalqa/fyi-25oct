    ...
    <div class="content container">
        ...
        <div class="row source_top_row">
            ...
            <div class="col-xs-6 col-sm-4 col-md-2">
                <a href="favorite_countries.php" class="btn btn-md btn-primary <?=(!stristr($page,'user_')) ? 'selected' : '';?>">Все страны</a>
            </div>
            <div class="col-xs-6 col-sm-4 col-md-2">
                <a href="user_favorite_countries.php" class="btn btn-md btn-primary <?=(stristr($page,'user_')) ? 'selected' : '';?>">Мои источники</a>
            </div>
        </div>

        <div class="container stats"><br><h1   >Список стран : </h1><hr></div>
        <div class="container-fluid">
         ...
         foreach($req as $country){ 
            ...
            <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
              <div class="card card-stats">
                ...
                <div class="card-footer">
                  <div class="stats">  
             <label class="containers">следовать
             <?php   
              if($log==true){//logged 
             ...
             <?php } 
              ...
              if(isset($_COOKIE['russian_sources'])){ //there is a cookie
                $data = json_decode($_COOKIE['russian_sources'], true);
          ...
          <?php }  //for  ?> 
             ...
             <input type="submit" class="green" name="Enregistrer" value="спасти" style="font-size: 26px; margin-right: 60px; " > 
            </form> 
          </div>   
        </div>   
            <?php  
         ...
         $GLOBALS['nb_sources_not_checked']+=v4Utilisteurs::nb_rss_sources_of_country($country['country']);
              ...
              if($GLOBALS['nb_sources_not_checked']>=5){//ok we can continue
                           ...
                           $idsOfCurrentCountry=v4Utilisteurs::get_ids_of_country($currentCountry); 
                            foreach($idsOfCurrentCountry as $row){  
                              ...
                              if($log==true){ 
                                ...
                                if(isset($_COOKIE['russian_sources'])){ 
                                      if(!in_array($id, $data)){ 
                                           array_push($data,$id); 
                                      }    
                                }else{//there is no cookie
                  ...
                  if($log==false){
                      if(isset($_COOKIE['russian_sources'])){
                      setcookie('russian_sources', json_encode($data),time()+31556926 ,'/');
                    }else{
                      setcookie('russian_sources', json_encode($notchecked),time()+31556926 ,'/'); 
                    }
                  }
                  ?>
                ...
                echo "<script> alert('Вы должны следовать по крайней мере пять источников'); </script>";
              }  
