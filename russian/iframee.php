		...
		<div class="container">
			<div class="row"> 
					...
					<div class="col-md-8 " style="margin-top: 40px; "> 
						...
						<form method="post" >
							<?php if ($log==false) { ?>
							<h3 class="xlogin"> Добро пожаловать<a href="login.php">&nbsp;авторизоваться &nbsp;</a>комментировать</h3> 
							<div class="line thin"></div>
						<?php } ?> 

						<p class="xtitleen" style="text-align: center;"> комментарии </p>
 						<p class="xtitleen"> уведомление : </p>
						 <span class="xtitle2en" >
						 	Опубликованные комментарии представляют не взгляды Chats Run и его сотрудников, а взгляды тех, кто их написал.
 						 </span> <br><br>

						<?php if ($log){ ?>
							<p style="font-size: 22px; ">
								<?php $countofrep=commentsModel::rss_countTotalOfComments($id_art_actuel);
								if ($countofrep<=1) echo $countofrep.' комментарий ';
								else echo $countofrep.' комментарии ';  ?>  &nbsp; &nbsp; &nbsp;
								<a onclick="f10()" style="cursor: pointer;"> Написать комментарий </a>
							</p> <hr>
						<?php } ?> 
						</form>
                            ...
                            !commentsModel::rss_comment_already_reported($comment['id'],$user_email) ){ ?> 
								<a href="<?php echo 'rss_report.php?link='.$clink.'&id='.$id_art_actuel.'&id_c='.$comment['id']; ?>">
									<button type="button" class="btn btn-danger xreporten1"
											 data-dismiss="modal">доклад
									</button>
								</a> 
						  <?php }else if(! commentsModel::rss_HisComment($comment['id'],$user_email) && 
                            commentsModel::rss_comment_already_reported($comment['id'],$user_email) ){ ?>
                            <button disabled="" type="button" class="btn btn-danger xreporten2" > 				доклад 
							</button> 
                                    <?php } ?>    
								  </div>  
								   <?php  } ?>
								  ...
								  <?php } else if(!commentsModel::rss_HisReply($reply['id'],$user_email) && 
								   commentsModel::rss_reply_already_reported($reply['id'],$user_email)){ ?> 
 									<button disabled="" type="button"
 										 class="btn btn-danger xreporten2" data-dismiss="modal">доклад
								     </button> 
 									<?php } ?> 
								    </div>   
								   <?php  } // log true ?>
					...
					<?php	if($log){  ?>
							<form style="padding-bottom: 50px;"  enctype="multipart/form-data" method="post" class="row"> 
								<div class="form-group col-md-12">
									<label for="message">комментарий <span class="required"></span></label>
									<textarea style="font-size: 18px;" maxlength="500" required="required" id="myTextField"  class="form-control" name="message"  placeholder="Написать комментарий ..."></textarea>
								</div>  
								<div class="image-upload form-group col-md-8">  
									 <label for="file-input" style="font-size: 18px;">
			                          Добавить изображение или видео &nbsp; : &nbsp; <img style="cursor: pointer;" width="15%" height="15%" src="images/cam.png"/>
			                           </label> 
								...
								<div class="form-group col-md-4"> 
									...
									<input value="отправить" name="send" type="submit" class="btn btn-primary form-control" style="font-size: 20px; padding-top:5px !important; margin-top: 0px !important">
								</div>  
							</form> 
								<?php  
							// comment 
                            ...
                            if($state_logo==1){  
                            	if($uploadOk==1){ 
                            		if (isset($_GET['r'])&&!empty($_GET['r'])) {
                            			...
                            			<?php  }else { ?>
                            				<script> 
                            					swal("Внимание!","Комментарий не добавлен!","warning"); 
                            				</script>
                            				<?php 	}  
                            		}
                            		/*
                            			...
                            			*/
                            			...
                            			$msg=null;  
                            			if($size_error==1) $msg.='Размер файла должен быть меньше чем '.$maxsize.' MB\n';
                            			if($checkexistedeja==1) $msg.='Файл уже существует, пожалуйста, измените имя файла и попробуйте снова\n'; 
                            			if($typeimage==1) $msg.='Извините, просто примите формат jpg, jpeg, png, gif, ac-3, mp3, mp4, flk, mkv, mpeg & webm\n'; 
                            			if($fin==1) $msg.='Извините, ошибка при загрузке файла.\n'; 
                            			$msg.="Комментарий не добавлен!";  
                            			?>
                            			<script>
                            				swal("Внимание!","<?php echo $msg;?>","warning")
                            			</script> 
                            			<?php 
                            		}
                            	}else{  
