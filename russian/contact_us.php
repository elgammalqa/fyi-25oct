...
<script async defer data-website-id="afc1b19c-5319-4097-8747-3b05933578c7" src="https://www.fyipress.net/umami.js"></script> 
	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			<div class="container"> 
			<h3 class="fcontact" >Свяжитесь с нами</h3>
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">Эл. адрес * : </label>
	                    <?php if($log){ ?>
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">предмет * : </label>
	                    <input required class="form-control" name="subject" />
                	</div>
				</div> 
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">сообщение * : </label>
...
<?php if (isset($_POST['send'])) {
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("сообщение","Вы не можете добавлять более 2 сообщений в день", "warning");</script>';
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    		echo '<script>swal("сообщение","Сообщение успешно отправлено", "success");</script>';
    		}else{
    		echo '<script>swal("сообщение","сообщение не было успешно отправлено", "warning");</script>';  
    		}
    	} 
   		 }else{//recap not ok
   		 	echo '<script>swal("робот","Проверьте, я не робот", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("робот","Проверьте, я не робот", "warning");</script>';
	    } 
