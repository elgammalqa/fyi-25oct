...
<div class="content container">
    ...
    <div class="container stats" style="display: none"><br>
        <h1 style="text-align: right;"> قائمة الدول </h1>
        <hr>
    </div>
        ...
        <form method="post" action="">
                    ...
                    <div class="col-xs-12 col-md-7 col-md-push-3">
                        <div class="card card-stats">
                            <div class="card-header card-header-default card-header-icon">
                                ...
                                <h5 class="h5">
                                    <a href="#" onclick="return false">
                                        <?php echo v4Utilisteurs::get_country_arabic_name(strtolower($c)); ?>
                                    </a>
                                </h5>
                            </div>
                            <div class="card-footer">
                                ...
                                foreach ($c_sources as $country) {
                                    ...
                                    <div class="row source_row">
                                        <span style="display: none"><?= 'ar-' . str_replace(' ', '__', $c) . '-' . str_replace(' ', '__', $source);
                                        ?></span>
            ...
            <div class="row">
                <div class="col-xs-12 text-center">
                    <input type="submit" name="Enregistrer" value=" حفظ ">
                </div>
            </div>
        </form>
    </div>
...
if ($log) {
    ...
    foreach ($req as $country) {
        ...
        echo $c.'<br>';
        $c_sources = $utilisateur->get_sources_of_country($c);
        foreach ($c_sources as $source){
            echo $source['source'].'<br>';
            $topic = 'ar-' . str_replace(' ', '__', $c) . '-' . str_replace(' ', '__', $source['source']);
            echo $topic.'<br>';
    ...
    foreach ($all_user_sources as $source) {
        $topic = 'ar-' . str_replace(' ', '__', $source['country']) . '-' . str_replace(' ', '__', $source['source']);
        ?><div style="display: none"><?=$topic;?><br></div><?php
        $fcm = new WebFCMToTopic();
        $fcm->unsubscribe($token, $topic);
    }
    ...
    foreach ($selected_sources as $src) {
        ...
        $topic = 'ar-' . str_replace(' ', '__', $country) . '-' . str_replace(' ', '__', $source);
        $fcm = new WebFCMToTopic();
