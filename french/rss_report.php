			...
			<div class="container">
				<div class="row"> 
                  ...
                  <div class="col-md-8">
		            ...
		            <div style="padding-left: 50px; padding-right: 50px;" >
		            	 <div class="form-group">
				            <label for="sel1">Type de rapport :</label>
				              <select required="required" name="typee" class="form-control" id="sel1"> 
				                <option value="Contenu sexuel" >Contenu sexuel</option>
				                <option value="Contenu violent" >Contenu violent</option>
				                <option value="Un mauvais mots" >Un mauvais mots</option>
				                <option value="Discours de haine" >Discours de haine</option>
				                <option value="abuse" >abuse</option>
				              </select>
				          </div>
				          <div class="form-group">
				            <label for="pwd">Autre :</label>
				            <textarea name="other" rows="5" cols="20" class="form-control" placeholder="Autre ....." ></textarea> 
				         </div>
				         <div class="modal-footer">
				          <button type="submit" name="back" class="btn btn-primary" data-dismiss="modal">Retour</button>
				          <button type="submit" name="send" class="btn btn-success" data-dismiss="modal">Envoyer</button>
				        </div>  
		            </div>
		            </form>
		            	<?php   
                           if (isset($_GET['id'])&&!empty( $_GET['id'])&&isset($_GET['link'])&&!empty( $_GET['link'])&&isset($_POST['back'])){ 
                           	$has_hashtag = strpos($_GET['link'], '#') !== false; 
							if($has_hashtag){
								$whatIWant = substr($_GET['link'], 0,strpos($_GET['link'], "#") );
								$cclink=$whatIWant;
							}else{
								$cclink=$_GET['link'];
							}  
		            		...
		            		<script type="text/javascript">
		                        window.location.replace("iframe.php?link=<?php echo $cclink; ?>&id=<?php echo $_GET['id']; ?>");
		            		</script>
		            <?php	}else if ((!isset($_GET['id'])||empty( $_GET['id'])||!isset($_GET['link'])||empty( $_GET['link']))&&isset($_POST['back'])){ ?>
		            		<script type="text/javascript">
		          ...
		          <?php   }
		            	...
		            	if (isset($_POST['send'])){ 
		            	 if (isset($_GET['id'])&&!empty( $_GET['link'])&&isset($_GET['link'])&&!empty( $_GET['id'])&&isset($_GET['id_c'])&&!empty($_GET['id_c'])&&isset($_POST['typee'])&&!empty($_POST['typee']) ) {  
		            	$clink=$_GET['link'];
		            	  ...
		            	  $report->setemail_user_abuse(reportModel::rss_getemail_user_abuse_c($id_comment));
		            	  ...
		            	  if (isset($_POST['other'])&&!empty($_POST['other'])) $report->setother($_POST['other']);  
		            	   ...
		            	   if ($report->rss_add_report()) { ?>   
                             <script> 
                             	window.location.replace("iframe.php?link=<?php echo $clink; ?>&id=<?php echo $_GET['id']; ?>");
                             </script>
                        <?php  }else{ ?> 
                        	<script> 
                        	      swal("Attention!","Le rapport n a pas été ajouté! Vous avez déjà signalé ce commentaire","warning");
                            </script> 
		           		<?php }  
		           	}//id c
  
		            if (isset($_GET['link'])&&!empty( $_GET['link'])&&isset($_GET['id'])&&!empty($_GET['id'])&&isset($_GET['id_r'])&&!empty($_GET['id_r'])&&isset($_POST['typee'])&&!empty($_POST['typee']) ) {
		            	$clink=$_GET['link'];
		            	$id_news=$_GET['id'];
		            	$id_reply=$_GET['id_r'];
		            	  $report=new reportModel(); 
		            	  $report->setid_news($id_news);
		            	  $report->setemail_user_report($user_email);
		            	  $report->setemail_user_abuse(reportModel::rss_getemail_user_abuse_r($id_reply));
