...
<script async defer data-website-id="afc1b19c-5319-4097-8747-3b05933578c7" src="https://www.fyipress.net/umami.js"></script> 
	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			<div class="container"> 
			<h3 class="fcontact" >Contactez nous</h3>
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">Le sujet * : </label>
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("Message","Vous ne pouvez pas ajouter plus de 2 messages par jour", "warning");</script>';
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    		echo '<script>swal("Message","Message envoyé avec succès", "success");</script>';
    		}else{
    		 echo '<script>swal("Message","le message n a pas été envoyé avec succès", "warning");</script>';
    		}
    	}  
   		 }else{//recap not ok
   		 	echo '<script>swal("Robot","Vérifiez que je ne suis pas un robot", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("Robot","Vérifiez que je ne suis pas un robot", "warning");</script>';
	    } 
