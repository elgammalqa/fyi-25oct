	...
	<body onload="f10()" class="skin-orange">
			...
			<div class="container">
				<div class="row">
						...
						<article class="article main-article"> 
							<div class="main"> 
								...
								<?php } ?>   
				 					<div class="xtypedate" >
										<span class="xdateen" >
										Publié  <?php real_news_time($news->getpubDate()); ?>
										</span> 
										<a class="xtypeen" >
					...
					<?php if(news_publishedModel::nbrnewsStartCountVideos2()>0){ ?>
						<div class="line"><div>Vous Pourriez Aussi Aimer</div></div>
						<div class="row">  
							 <?php  	// news videos   
					...
					<div class="col-md-6 col-sm-6 col-xs-12"  >
						...
						<form method="post" >
						<?php if ($log==false) { ?>
							<h3 class="xlogin">  Veuillez <a href="login.php">
							&nbsp;vous connecter&nbsp;</a> pour commenter </h3>
							<div class="line thin"></div>
						<?php } ?> 

						<p class="xtitleen" style="text-align: center;"> commentaires </p>
 						<p class="xtitleen"> Remarque : </p>
						 <span class="xtitle2en" >
						 	Les commentaires publiés ne représentent pas les vues de Chats Run et de ses employés, mais plutôt les vues de ceux qui les ont écrites.
 						 </span> <br><br>

						<?php if ($log){ ?>
							<p style="font-size: 22px; ">
								<?php $countofrep=commentsModel::countTotalOfComments($id_art_actuel);
								if ($countofrep<=1) echo $countofrep.' commentaire ';
								else echo $countofrep.' commentaires ';  ?>  &nbsp; &nbsp; &nbsp;
								<a onclick="f10()" style="cursor: pointer;"> 
							...
							!commentsModel::comment_already_reported($comment['id'],$user_email) ){ ?>
								<a href="<?php echo 'report.php?id='.$id_art_actuel.'&id_c='.$comment['id']; ?>">
									<button type="button" class="btn btn-danger xreporten1"
											 data-dismiss="modal">Signaler
									</button>
								</a>
							<?php } else if(!commentsModel::HisComment($comment['id'],$user_email) &&
						 		commentsModel::comment_already_reported($comment['id'],$user_email) ){ ?>
								<button disabled="" type="button" class="btn btn-danger xreporten2" >Signaler 
								</button>
							<?php } ?> 
							   ...
							   <?php if ($log){?>
								...
								!commentsModel::reply_already_reported($reply['id'],$user_email)){ ?>
								   ...
								   <?php }else if(!commentsModel::HisReply($reply['id'],$user_email) && 
								     commentsModel::reply_already_reported($reply['id'],$user_email)){ ?>
								        <button disabled="" type="button" class="btn btn-danger xreporten2" data-dismiss="modal">Signaler
								        </button>
								         <?php } ?> 
						...
						<?php	if($log){  ?>
							<form style="padding-bottom: 50px;"  enctype="multipart/form-data" method="post" class="row"> 
								<div class="form-group col-md-12">
									<label for="message">Commentaire <span class="required"></span></label>
									<textarea style="font-size: 18px;" maxlength="500" required="required" id="myTextField"  class="form-control" name="message"  placeholder="Ecrivez votre commentaire ..."></textarea>
								</div>  
								<div class="image-upload form-group col-md-8">  
									 <label for="file-input" style="font-size: 18px;">
			                          Ajouter une image ou une vidéo &nbsp; : &nbsp; <img style="cursor: pointer;" width="15%" height="15%" src="images/cam.png"/>
			                           </label> 
								...
								<div class="form-group col-md-4"> 
									...
									<input value="Envoyer" name="send" type="submit" class="btn btn-primary form-control" style="font-size: 20px; padding-top:5px !important; margin-top: 0px !important">
								</div>  
							</form> 
           ...
           if (isset($_GET['r'])&&!empty($_GET['r'])) {
                ...
                $imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);  
                                ...
                                if($uploadOk==1){ 
                                	if (isset($_GET['r'])&&!empty($_GET['r'])) {
                                     ...
                                     <?php  }else   echo '<script>
                                                 swal("Attention!","Le commentaire n est pas ajouté !","warning")
                                            </script> '; 
                                     ...
                                     <?php   }else   echo '<script>
                                            ...
                                            $msg=null;  
                                          if($size_error==1) $msg.='La taille du fichier doit être inférieure à '.$maxsize.' MB\n';
                                             if($checkexistedeja==1) $msg.='Le fichier existe déjà, changez le nom du fichier et essayez à nouveau\n'; 
                                            if($typeimage==1) $msg.='Désolé, les formats acceptés jpg, jpeg, png, gif, ac3, mp3, mp4, flk, mkv, mpeg et webm\n'; 
                                            if($fin==1) $msg.='Désolé, erreur lors du téléchargement du fichier..\n'; 
                                            $msg.="Le commentaire n est pas ajouté";  
                                            ?>
                                            <script>
					...
					<div class="col-md-4 sidebar" id="sidebar"> 
						...
						<aside>
							<br>
							<h1 class="aside-title">Dernières Actualités</h1>
		...
		<script src="../js/e-magz.js"></script>
		 <script type="text/javascript">
 			...
 			function f11(){
 				window.location.replace('detaill.php?id=<?php echo $id_art_actuel; ?>'); 
 			}
