	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();" lang="ar" dir="rtl" >
		...
		<section class="category" style="margin-bottom: 2%; padding: 10px;" style="float: right;" >
			...
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-md-offset-4 col-xs-12 col-sm-12"> 
	                    <label class="fullname"> ای میل * : </label>
	                    <?php if($log){ ?>
...
<?php if (isset($_POST['send'])) {
    	...
    	if(nb_messages_today($email)>1){  
    		echo '<script>swal("پیغام","آپ فی دن 2 سے زیادہ پیغامات شامل نہیں کر سکتے ہیں", "warning");</script>'; 
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    		echo '<script>swal("پیغام","پیغام کامیابی سے بھیجا گیا", "success");</script>';
    		}else{
    		echo '<script>swal("پیغام","پیغام کامیابی سے نہیں بھیج دیا گیا تھا", "warning");</script>'; 
    		}
    	}  
   		 }else{//recap not ok
   		 	echo '<script>swal("روبوٹ","چیک کریں میں روبوٹ نہیں ہوں", "warning");</script>';
   		 }
	    }else{//not checked 
	      echo '<script>swal("روبوٹ","چیک کریں میں روبوٹ نہیں ہوں", "warning");</script>';
	    } 
