...
<script async defer data-website-id="afc1b19c-5319-4097-8747-3b05933578c7" src="https://www.fyipress.net/umami.js"></script> 
	...
	<body  class="skin-orange" onLoad="document.myf.name.focus();">
		...
		<section class="category" style="margin-bottom: 2%" >
			<div class="container"> 
			<h3 class="fcontact" >Contactanos</h3>
			<form method="post" name="myf"  id="myf"  >
				...
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">Número de teléfono : </label>
	                    <input class="form-control" name="phone" />
                	</div>  
				</div>
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">tema * : </label>
	                    <input required class="form-control" name="subject" />
                	</div>
				</div> 
				<div class="row" >
					<div class="col-md-8 col-xs-12 col-sm-12"> 
	                    <label for="fullname">mensaje * : </label>
...
<?php if (isset($_POST['send'])) {
    	...
    	if(nb_messages_today($email)>1){ 
    		echo '<script>swal("mensaje","No puedes agregar más de 2 mensajes por día.", "warning");</script>'; 
    	}else{  
    		if(add_message($email,$name,$subject,$message,$phone)){
    		echo '<script>swal("mensaje","mensaje enviado con éxito", "success");</script>';
    		}else{
    		echo '<script>swal("mensaje","el mensaje no fue enviado exitosamente", "warning");</script>';  
    		}
    	} 
   		 }else{//recap not ok
   		 	echo '<script>swal("Robot","Compruebe que no soy un robot", "warning");</script>';
   		 } 
	    }else{//not checked 
	      echo '<script>swal("Robot","Compruebe que no soy un robot", "warning");</script>';
	    } 
