    ...
    <div class="content container">
        ...
        <div class="row source_top_row">
            ...
            <div class="col-xs-6 col-sm-4 col-md-2">
                <a href="favorite_countries.php" class="btn btn-md btn-primary <?=(!stristr($page,'user_')) ? 'selected' : '';?>">Todos los países</a>
            </div>
            <div class="col-xs-6 col-sm-4 col-md-2">
                <a href="user_favorite_countries.php" class="btn btn-md btn-primary <?=(stristr($page,'user_')) ? 'selected' : '';?>">Mis fuentes</a>
            </div>
        </div>

        <div class="container stats"><br><h1   >Listado de paises : </h1><hr></div>
        <div class="container-fluid">
         ...
         foreach($req as $country){ 
          ...
          ?> 
            <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
             ...
             <?php } 
              ...
              if(isset($_COOKIE['spanish_sources'])){ //there is a cookie
                $data = json_decode($_COOKIE['spanish_sources'], true);
          ...
          <?php }  //for  ?> 
             ...
             <input type="submit" class="green" name="Enregistrer" value="salvar" style="font-size: 26px; margin-right: 60px; " > 
            </form> 
          </div>   
        </div>   
            <?php  
         ...
         $GLOBALS['nb_sources_not_checked']+=v4Utilisteurs::nb_rss_sources_of_country($country['country']);
              ...
              if($GLOBALS['nb_sources_not_checked']>=5){//ok we can continue
                           ...
                           $idsOfCurrentCountry=v4Utilisteurs::get_ids_of_country($currentCountry); 
                            foreach($idsOfCurrentCountry as $row){  
                              ...
                              if($log==true){ 
                                ...
                                if(isset($_COOKIE['spanish_sources'])){ 
                                      if(!in_array($id, $data)){ 
                                           array_push($data,$id); 
                                      }    
                                }else{//there is no cookie
                  ...
                  if($log==false){
                      if(isset($_COOKIE['spanish_sources'])){
                      setcookie('spanish_sources', json_encode($data),time()+31556926 ,'/');
                    }else{
                      setcookie('spanish_sources', json_encode($notchecked),time()+31556926 ,'/'); 
                    }
                  }
                  ?>
                ...
                echo "<script> alert('Debes seguir Al menos cinco fuentes.'); </script>";
              }  
